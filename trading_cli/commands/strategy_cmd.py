...
def create(name: str, params: str | None, stop_loss: float, take_prob: float):
    ...
    if name in BUILTIN_STRATEGIES:
        console.print(
            f"[yellow]'{name}' is a built-in strategy. Registering...[/yellow]"
        )
...
def list():
    ...
    if BUILTIN_STRATEGIES:
        ...
        for name, cls in BUILTIN_STRATEGIES.items():
            table.add_row(name, cls.__name__, cls.__doc__ or "")
        console.print(table)
    else:
    ...
    if custom:
        ...
        for name in custom:
            ...
            table.add_row(
                name, f"SL:{config.stop_loss_pct}% TP:{config.take_profit_pct}%"
            )
        console.print(table)
    else:
...
def show(name: str):
    ...
    custom_config = registry.get(name)
    if not custom_config:
        console.print(f"[red]Strategy '{name}' not found.[/red]")
        return

    console.print(f"\n[green]Strategy:[/green] {custom_config.name}")
    console.print(f"[yellow]Description:[/yellow] {custom_config.description}")
    console.print(f"[yellow]Enabled:[/yellow] {custom_config.enabled}")
    console.print(f"[yellow]Stop Loss:[/yellow] {custom_config.stop_loss_pct}%")
    console.print(f"[yellow]Take Profit:[/yellow] {custom_config.take_profit_pct}%")
    console.print(
        f"[yellow]Position Size:[/yellow] {custom_config.position_size * 100}%"
    )
    console.print(
        f"[yellow]Commission Rate:[/yellow] {custom_config.commission_rate * 100}%"
    )
