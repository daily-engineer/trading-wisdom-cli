...
def _get_signal(rsi: float, macd_hist: float, price: float, bb_upper: float, bb_lower: float) -> tuple[str, str]:
...
@click.option("--days", "-d", type=int, default=60, help="Days of historical data (default: 60)")
@click.option("--limit", "-l", type=int, default=10, help="Rows to display (default: 10)")
...
@analyze.command()
@click.argument("symbol")
@click.option("--period", "-p", type=int, default=20, help="Lookback period (default: 20)")
def summary(symbol: str, period: int):
    ...
    content.append(f"{price:.2f} ({price_change:+.2f}%)\n", style="green" if price_change > 0 else "red")
